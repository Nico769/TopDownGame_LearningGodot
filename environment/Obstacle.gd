# Use the tool keyword to let this script run in the
# Godot Editor. This is useful here for our _update function
tool
extends StaticBody2D

enum Items {barrelBlack_side, barrelBlack_top,
            barrelGreen_side, barrelGreen_top,
            barrelRed_side, barrelRed_top,
            barrelRust_side, barrelRust_top,
            barricadeMetal, barricadeWood,
            fenceRed, fenceYellow,
            sandbagBeige, sandbagBeige_open,
            sandbagBrown, sandbagBrown_open,
            treeBrown_large, treeBrown_small,
            treeGreen_large, treeGreen_small}
...
# Helper function to update the sprite and colShape
# according to the selected Item in the Inspector menu
func _update(_new_type):
    type = _new_type
    # If the script is not running in the editor..
    if !Engine.editor_hint:
        #..wait for the tree_entered signal being launched
        # by Godot and then resume executing this function.
        # Read more at "Coroutines with yield" section
        # in the docs.
